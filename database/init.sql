...
CREATE TABLE publishers ( id SERIAL PRIMARY KEY, name VARCHAR(255) UNIQUE, logo VARCHAR(500) UNIQUE );

INSERT INTO publishers (name, logo) VALUES ('aljazera', 'somelink.com');

INSERT INTO publishers (name, logo) VALUES ('haarz', 'url.com');

CREATE TABLE newspapers( id SERIAL UNIQUE, published_date DATE, publisher_id INTEGER, CONSTRAINT fk_publisher FOREIGN KEY(publisher_id) REFERENCES publishers(id), PRIMARY KEY( publisher_id, published_date ) );

INSERT INTO newspapers (published_date, publisher_id) VALUES ('2021-09-05', '2');

CREATE TABLE newspaper_pages( id SERIAL PRIMARY KEY, newspaper_id INTEGER REFERENCES newspapers(id), page_number INTEGER, name VARCHAR(255) );
...
CREATE TABLE overlay_coords( id SERIAL PRIMARY KEY, newspaper_id INTEGER REFERENCES newspapers(id), coords TEXT[] );


INSERT INTO overlay_coords（newspaper_id ,coords） VALUES （'1',
'[{"x":4756.60681369538 "y":330.51214148731304 "width":378.2821562709105 "height":287.21052721097345 "degrees":0} {"x":5330.046355699557 "y":32.4140969014465 "width":176.0542458985219 "height":412.9440855428983 "degrees":0} {"x":5503.883110580155 "y":454.2190584381016 "width":185.9329866218177 "height":343.8565170254213 "degrees":0}]'）;

CREATE TABLE admins( id SERIAL PRIMARY KEY, email VARCHAR(255) UNIQUE, password VARCHAR(255), created_at DATE DEFAULT CURRENT_TIMESTAMP );
